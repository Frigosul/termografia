...
const searchDataSchema = z.object({
  ...
  searchParams: z.string().refine(value => searchParams.includes(value), {
    message: "Parâmetro de pesquisa inválido. Escolha um dos valores disponíveis.",
  }),
...
export function TableManagedData() {
  ...
  const [data, setData] = useState<RowData[]>([
    { id: 1, dateTime: "17/08/2024 - 12:23", temperature: 2.5, status: 'Temperatura Integrada' },
    { id: 2, dateTime: "17/08/2024 - 12:23", temperature: 3.5, status: 'Temperatura Integrada' },
    { id: 3, dateTime: "17/08/2024 - 12:23", temperature: 4.5, status: 'Temperatura Integrada' },
    { id: 4, dateTime: "17/08/2024 - 12:23", temperature: 6.6, status: 'Temperatura Integrada' },
    { id: 5, dateTime: "17/08/2024 - 12:23", temperature: 11.0, status: 'Temperatura Integrada' },
    { id: 6, dateTime: "17/08/2024 - 12:23", temperature: 11.0, status: 'Temperatura Integrada' },

  ]);

  const [editCell, setEditCell] = useState<{ rowId: number | null; field: keyof RowData | null }>({
    rowId: null,
    field: null,
  });
  ...
  function handleDoubleClick(rowId: number, field: keyof RowData, currentValue: string | number) {
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>, rowId: number, field: keyof RowData) {
    if (e.key === 'Enter') {
  ...
  return (
    <div className="flex flex-col w-full items-center justify-between border border-card-foreground rounded-md overflow-hidden">
      <div className="flex justify-between w-full border-b border-card-foreground">
        ...
        <form className="flex items-center justify-center gap-2" onSubmit={handleSubmit(handleSearchData)} >
          <Input {...register('hour')} placeholder="Hora" type="time" className='w-22' />
          <Controller
            name="searchParams"
            control={control}
            render={({ field: { onChange, value, ref } }) => (
              <Select onValueChange={onChange} value={value} >
                <SelectTrigger ref={ref} className="dark:bg-slate-900 w-[20rem]">
                ...
                <SelectContent className='w-[20rem]'>
                  ...
                  <SelectItem value="greaterOrEqual">Maior ou igual à</SelectItem>
                </SelectContent>
              </Select>
            )}
          />
          <Input {...register('temperature')} placeholder="ºC" className='w-20' />
          <Button variant="default" className="flex items-center justify-center rounded-none hover:bg-slate-300">
            <Search />
          </Button>
        </form>
      </div>
      <Table className="border border-collapse rounded-md">
        <TableHeader className='bg-card' >
          <TableRow>
            <TableHead className="border text-card-foreground w-64 text-center" >Data / Hora - HH:mm</TableHead>
            <TableHead className="border text-card-foreground w-20 text-center" >Temperatura</TableHead>
            <TableHead className="border text-card-foreground" >Status da alteração</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {data.map(row => {
            return (
              <TableRow key={row.id} className="odd:bg-white odd:dark:bg-slate-950 even:bg-slate-50 even:dark:bg-slate-900 ">
                <TableCell
                  className="border text-center"
                  onDoubleClick={() => handleDoubleClick(row.id, 'dateTime', row.dateTime)}
                >
                  {editCell.rowId === row.id && editCell.field === 'dateTime' ? (
                    <input
