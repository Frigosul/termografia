    ...
    return (
      <div className="border rounded-md p-2">
        <div className="flex flex-col items-center justify-center py-2">
          ...
          <p className="text-center text-3xl mt-3 text-primary">
            {Number(temperature).toFixed(1)} {type === 'temp' ? '°C' : 'bar'}
          </p>
        </div>
        <div className="flex justify-between lg:p-2 flex-wrap">
          <span className={`text-xs ${mode === 'simple' ? 'font-light' : 'font-normal'}  lg:text-sm flex items-center justify-center gap-3`}>
            DEGE.
            <div
          ...
          <span className="text-xs  font-normal lg:text-sm flex items-center justify-center gap-3">
            VENT.
            <div
              className={`size-3 ${mode === 'simple' && 'lg:size-3'} rounded-full ${status.includes('vent') ? 'bg-emerald-500' : 'bg-zinc-400'}`}
            />
          </span>
          <span className="text-xs  font-normal lg:text-sm flex items-center justify-center gap-3">
            RESF.
            <div
              className={`size-3 ${mode === 'simple' && 'lg:size-3'} rounded-full ${status.includes('resf') ? 'bg-emerald-500' : 'bg-zinc-400'}`}
            />
          </span>
          <span className="text-xs  font-normal lg:text-sm flex items-center justify-center gap-3">
            PORT.
            <div
              className={`size-3 ${mode === 'simple' && 'lg:size-3'} rounded-full ${status.includes('port') ? 'bg-emerald-500' : 'bg-zinc-400'}`}
            />
          </span>
        </div>
      </div>
