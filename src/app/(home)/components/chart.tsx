...
const Chart = memo(function Chart({ dataChart: { name, type, temperature, pressure, status, isSensorError, error, minValue, maxValue } }: ChartProps) {

  const { appearanceMode } = useApperanceStore()
  const valueInPercent = Math.min(
    Math.max(
      ((type === 'press' ? pressure : temperature) - minValue) / (maxValue - minValue) * 100,
      0
    ),
    100
  );
  const colorInPercent = valueInPercent >= 90 ? 'text-red-600' : valueInPercent >= 60 ? 'text-yellow-600' : 'text-primary'
  const data = [
    { value: valueInPercent }, // Filled part
    { value: 100 - valueInPercent }, // Unfilled part
  ]
  const COLORS = valueInPercent >= 90 ? ['#ef4444', '#93b1e4'] : valueInPercent >= 60 ? ['#eab308', '#93b1e4'] : ['#2178db', '#93b1e4']
  ...
  useMemo(() => {
    ...
    if (valueInPercent <= 10 && lastWarning.current !== 'low') {
      showTemperatureWarning(name, `${type === 'press' ? 'Pressão' : 'Temperatura'} em ${name}, próxima do mínimo permitido`);
      lastWarning.current = 'low'
    } else if (valueInPercent >= 90 && lastWarning.current !== 'high') {
      showTemperatureWarning(name, `${type === 'press' ? 'Pressão' : 'Temperatura'} em ${name}, próxima do máximo permitido`);
      lastWarning.current = 'high'
    } else if (valueInPercent > 10 && valueInPercent < 90 && lastWarning.current !== null) {
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <div className="flex flex-col text-center  mt-[-9.5rem]">
            ...
            <p className={`text-center text-2xl ${colorInPercent}`}>
              {!isSensorError && !error && Number(type === 'press' ? pressure : temperature).toFixed(1)} {!isSensorError && !error ? (type === 'temp' ? '°C' : 'bar') : <span className='flex w-28 text-base text-red-700 dark:text-red-500 flex-wrap'>{isSensorError ? 'Erro de sensor' : error && 'Falha de comunicação'}</span>}
            </p>
          </div>
        ...
        <div className="flex items-center justify-center gap-2">
          <div className="flex flex-col h-20 justify-between">
            ...
            <p className={`text-center text-3xl ${colorInPercent} `}>
              {!isSensorError && !error ? Number(type === 'press' ? pressure : temperature) : <span className='flex w-28 text-base text-red-700 dark:text-red-500 flex-wrap'>{isSensorError ? 'Erro de sensor' : error && 'Falha de comunicação'}</span>}
            </p>
