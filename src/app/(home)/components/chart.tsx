...
const Chart = memo(function Chart({ dataChart: { name, type, temperature, status, isSensorError, error, minValue, maxValue } }: ChartProps) {
  ...
  useMemo(() => {
    ...
    if (temperatureInPercent <= 10 && lastWarning.current !== 'low') {
      showTemperatureWarning(name, `Temperatura em ${name}, próxima do mínimo permitido`);
      lastWarning.current = 'low'
    } else if (temperatureInPercent >= 90 && lastWarning.current !== 'high') {
      showTemperatureWarning(name, `Temperatura em ${name}, próxima do máximo permitido`);
      lastWarning.current = 'high'
    } else if (temperatureInPercent > 10 && temperatureInPercent < 90 && lastWarning.current !== null) {
