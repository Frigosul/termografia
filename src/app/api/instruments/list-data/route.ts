...
export async function POST(req: NextRequest) {
  ...
  if (union) {
    const temp = union.firstInstrument.temperatures.length > 1 ? union.firstInstrument.temperatures : union.secondInstrument.temperatures
    const press = union.firstInstrument.pressures.length > 1 ? union.firstInstrument.pressures : union.secondInstrument.pressures
    const chartTemperature = filterByInterval({ data: temp, intervalMinutes: graphVariation, key: "temperature" });
    const chartPressure = filterByInterval({ data: press, intervalMinutes: graphVariation, key: "pressure" });
    const response: any = {
      id: union.id,
      name: union.name,
  ...
  const chartTemperature = filterByInterval({ data: data.temperatures, intervalMinutes: graphVariation, key: "temperature" });
  const chartPressure = filterByInterval({ data: data.pressures, intervalMinutes: graphVariation, key: "pressure" });
  ...
  if (tableVariation) {
    const tableTemperatureRange = filterByInterval({ data: data.temperatures, intervalMinutes: tableVariation, key: "temperature" });
    response.tableTemperatureRange = tableTemperatureRange.map(temp => ({
      id: temp.temperature.id,
      time: temp.temperature.createdAt.toISOString(),
      temperature: temp.temperature.editValue,
